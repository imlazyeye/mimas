...
- The `export-api` feature of `mimas`, on by default, writes the host's API to `target/mimas/api.json` when the host runs from its cargo target dir. `mimas::write_api` writes it anywhere else. This allows the LSP to give you type support and documentation for your native types, and will also serve our reference documentation efforts in an upcoming release. Turn the automatic write off with `default-features = false`.
